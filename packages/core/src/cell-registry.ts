...
function resolveDeclaredPath(
  ...
  options: {
    ...
    /** Reported when the path's *real* location is outside the project root. */
    readonly outsideCode: ConfigDiagnosticCode;
    readonly label: string;
    readonly requireEntryFiles: boolean;
  },
  out: ConfigDiagnostic[],
): ResolvedEntry | undefined {
  const { root, path, code, missingCode, outsideCode, label, requireEntryFiles } = options;
  ...
  if (requireEntryFiles) {
    // Filesystem validation is on, so the *real* location is the location that
    // counts. `existsSync`/`statSync` follow links while the containment check
    // above is lexical on the authored path, so `./cells/x.ts` could be a link —
    // or sit behind a linked directory — whose real target is outside the
    // project and still pass. Comparing real paths closes that hole, which is
    // the clean-checkout invariant the check exists for.
    const realRoot = realpathSync(root);
    const realPath = realpathSync(absolute);
    if (!isInsideProjectRoot(realRoot, realPath)) {
      out.push(
        diag(
          outsideCode,
          path,
          `${label} "${authored}" resolves outside the project root through links (real path: ${realPath}). Keep managed source inside the project so a clean checkout means the same thing.`,
        ),
      );
      return undefined;
    }
...
function validateCell(
  ...
  const entry = resolveDeclaredPath(
    raw.entry,
    {
      ...
      outsideCode: "entry-outside-project-root",
      label: "Cell entry",
  ...
  const fixture = resolveDeclaredPath(
    raw.fixture,
    {
      ...
      outsideCode: "fixture-outside-project-root",
      label: "Cell fixture",
      requireEntryFiles: options.requireEntryFiles,
    },
    out,
...
export interface TargetClaim {
  ...
  /** Authoritative coordinates: the two fields `api.page.setCells` writes to. */
  readonly pageName: string;
  readonly cell: string;
  /**
   * A locator key the claimant already carries, when it has one.
   *
   * Never trusted. The guard recomputes the identity from `pageName` and `cell`
   * and refuses a stored key that disagrees with it, because a registry may
   * arrive from a host, a cache or a deserialized payload: two claims carrying
   * *different* stored keys for the *same* destination would otherwise look
   * distinct and let the second write silently overwrite the first.
   */
  readonly locatorKey?: string;
}

/**
 * Fails when two claims point at one Forguncy target — and whenever a claim's
 * carried key contradicts its own coordinates.
 *
 * Identity is recomputed from `pageName` and `cell` here, at the boundary, and
 * not read from `locatorKey`: the coordinates are what the platform acts on, so
 * they are the only thing a guard standing between a plan and a mutation may
 * trust. Two names for one destination mean the second write silently
 * overwrites the first, so this has to fail *before* any mutation — after a
 * partial sync there is no safe rollback.
 */
export function assertDistinctTargetClaims(claims: readonly TargetClaim[], context = "cell registry"): void {
  ...
  for (const claim of claims) {
    const recomputed = targetLocatorKey({ pageName: claim.pageName, cell: claim.cell });

    if (claim.locatorKey !== undefined && claim.locatorKey !== recomputed) {
      diagnostics.push(
        diag(
          "locator-key-mismatch",
          `cells.${claim.cellId}.target`,
          `Carried locator key ${JSON.stringify(claim.locatorKey)} does not match the identity recomputed from its own coordinates (${recomputed}); the registry is forged or stale, so it is refused before any project mutation.`,
        ),
      );
    }

    const group = byLocator.get(recomputed);
    if (group === undefined) {
      byLocator.set(recomputed, [claim.cellId]);
    } else {
 ...
 * receive one from a host, a cached snapshot or a deserialized payload — so the
 * identity is recomputed from each cell's coordinates rather than read back from
 * the stored `locatorKey`.
 */
export function assertUniqueTargets(registry: CellRegistry): void {
  assertDistinctTargetClaims(
    registry.cells.map(cell => ({
      cellId: cell.id,
